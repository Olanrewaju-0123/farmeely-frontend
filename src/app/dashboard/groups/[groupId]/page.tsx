...
export default function GroupDetailsPage({ params }: GroupDetailsPageProps) {
  ...
  if (isError || !groupDetails) {
    ...
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center">
        <h2 className="text-xl font-semibold text-red-600">Group not found or an error occurred.</h2>
  ...
  const progressPercentage = (groupDetails.slotTaken / groupDetails.totalSlot) * 100
  ...
  return (
    <div className="px-4 py-6 sm:px-0">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">{groupDetails.groupName}</h1>
        <p className="text-gray-600 mt-2">{groupDetails.description}</p>
      </div>
      <Card className="w-full max-w-2xl mx-auto">
        <CardHeader>
          <CardTitle>Investment Details</CardTitle>
          <CardDescription>Overview of this group's progress and details.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {groupDetails.livestock && (
            <div className="space-y-2">
              <h3 className="text-lg font-semibold">Livestock: {groupDetails.livestock.name}</h3>
              ...
              <p className="text-muted-foreground">{groupDetails.livestock.description}</p>
              <p className="text-lg font-bold">
                Total Livestock Value: ₦{groupDetails.livestock.price.toLocaleString()}
              </p>
            </div>
          )}
          <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-green-600">₦{groupDetails.slotPrice.toLocaleString()}</div>
          ...
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            <div className="flex items-center">
              ...
              Created by: {groupDetails.created_by} {/* You might want to fetch creator's name */}
            </div>
            <div className="flex items-center">
              <TrendingUp className="w-4 h-4 mr-1" />₦{groupDetails.finalSlotPriceTaken.toLocaleString()} raised
            </div>
          </div>
          <div className="flex justify-center mt-6 gap-4">
            ...
            {groupDetails.totalSlotLeft > 0 && groupDetails.status === "active" && (
              <Button onClick={handleJoinGroupClick} className="bg-green-600 hover:bg-green-700 text-white">
                Join Group
              </Button>
            )}
          </div>
      ...
      {groupDetails && (
        <JoinGroupModal groupId={groupDetails.group_id} isOpen={isJoinModalOpen} onClose={handleCloseJoinModal} />
      )}
    </div>
  )
}
