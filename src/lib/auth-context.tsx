...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchUserProfile = useCallback(async (authToken: string): Promise<ApiResponse<User>> => {
    try {
      ...
      if (userProfileResponse.status === "success" && userProfileResponse.data) {
        ...
        console.error("AuthContext: Failed to fetch user profile:", userProfileResponse.message)
        return {
          status: "error",
      ...
      return { status: "error", message: error.message || "Network error fetching user profile." }
    }
  }, [])
  ...
  useEffect(() => {
    const loadAuthData = async () => {
      try {
        ...
        console.error("AuthContext: Error loading auth data from localStorage:", error)
        localStorage.removeItem("token")
        localStorage.removeItem("user")
        setToken(null)
        setUser(null)
  ...
  const login = useCallback(
    async (payload: LoginPayload): Promise<ApiResponse<{ user: User; token: string }>> => {
      setIsLoading(true)
      try {
        ...
        if (response.status === "success" && response.token) {
          ...
          if (userProfileResponse.status === "success" && userProfileResponse.data) {
            ...
            console.error("AuthContext: Failed to fetch user profile after login:", userProfileResponse.message)
            toast({
              title: "Login Successful (Profile Fetch Failed)",
              description: userProfileResponse.message || "Logged in, but failed to load profile.",
          ...
          toast({
            title: "Login Failed",
            description: response.message || "Invalid credentials or no token received.",
            variant: "destructive",
          })
          return { status: "error", message: response.message || "Login failed: No token received." }
        }
      } catch (error: any) {
        ...
        return { status: "error", message: error.message || "Network error during login." }
      } finally {
        setIsLoading(false)
      }
    },
      ...
      try {
        ...
        if (response.status === "success") {
          toast({
            ...
            description: response.message || "Account created successfully. Please verify your email.",
            variant: "default",
          })
        ...
        return { status: "error", message: error.message || "Network error during signup." }
      } finally {
        setIsLoading(false)
      }
    },
  ...
  const forgotPassword = useCallback(
    async (payload: ForgotPasswordPayload): Promise<ApiResponse> => {
      ...
      try {
        ...
        return { status: "error", message: error.message || "Network error during forgot password." }
      } finally {
        setIsLoading(false)
      }
    },
  ...
  const resetPassword = useCallback(
    async (payload: ResetPasswordPayload): Promise<ApiResponse> => {
      ...
      try {
        ...
        return { status: "error", message: error.message || "Network error during password reset." }
      } finally {
        setIsLoading(false)
      }
    },
  ...
  const verifyEmail = useCallback(
    async (payload: VerifyEmailPayload): Promise<ApiResponse<{ user: User; token: string }>> => {
      setIsLoading(true)
      try {
        ...
        if (response.status === "success" && response.token) {
          ...
          if (userProfileResponse.status === "success" && userProfileResponse.data) {
            ...
            toast({
              ...
              description: userProfileResponse.message || "Email verified, but failed to load profile.",
              variant: "destructive",
            })
            router.push("/dashboard")
            return {
          ...
          toast({
            ...
            description: response.message || "Invalid OTP or an error occurred.",
            variant: "destructive",
          })
          return { status: "error", message: response.message || "Verification failed." }
        }
      } catch (error: any) {
        toast({
          title: "Error",
          description: error.message || "Network error during email verification.",
          variant: "destructive",
        })
        return { status: "error", message: error.message || "Network error during email verification." }
      } finally {
        setIsLoading(false)
      }
    },
  ...
  const value = React.useMemo(
    ...
    [user, token, isLoading, login, signup, logout, forgotPassword, resetPassword, verifyEmail, fetchUserProfile],
  )
