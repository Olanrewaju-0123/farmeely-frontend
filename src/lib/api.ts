...
class ApiClient {
  ...
  resetPassword(payload: ResetPasswordPayload): Promise<ApiResponse> {
    return this.request("/user/forgot-password/complete", "POST", payload);
  }
  ...
  resendOtp(email: string): Promise<ApiResponse> {
    return this.request(`/users/resend-otp/${email}`, "PATCH");
  }

  // Group Management
   startCreateGroup(payload: StartCreateGroupPayload, authToken: string): Promise<ApiResponse<{ group_id: string }>> {
  return this.request("/groups/create/start", "POST", payload, undefined, authToken)
}
...
completeCreateGroup(payload: { groupId: string; paymentMethod: string; paymentReference?: string }, authToken: string): Promise<ApiResponse<Group>> {
  ...
  getActiveGroups(authToken: string): Promise<ApiResponse<Group[]>> {
    return this.request("/groups/active", "GET", undefined, undefined, authToken)
  }

  // Get user's groups
  getMyGroups(authToken: string): Promise<ApiResponse<Group[]>> {
    return this.request("/groups/my-groups", "GET", undefined, undefined, authToken)
  }
  ...
  joinGroup(groupId: string, payload: JoinGroupPayload, authToken: string): Promise<ApiResponse> {
    return this.request(`/groups/${groupId}/join/start`, "POST", payload, undefined, authToken)
  }

  // Complete join group
  completeJoinGroup(paymentReference: string, authToken: string): Promise<ApiResponse> {
    return this.request(`/groups/join/complete/${paymentReference}`, "POST", undefined, undefined, authToken)
  }
  ...
  createLivestock(payload: any, authToken: string): Promise<ApiResponse<Livestock>> {
  ...
  getLivestockById(livestockId: string, authToken: string): Promise<ApiResponse<Livestock>> {
    return this.request(`/livestocks/${livestockId}`, "GET", undefined, undefined, authToken)
  }
  ...
  startWalletFunding(payload: WalletFundingPayload, authToken: string): Promise<ApiResponse> {
    return this.request("/wallet/funding/start", "POST", payload, undefined, authToken)
  }

  // Complete wallet funding
  completeWalletFunding(reference: string, authToken: string): Promise<ApiResponse> {
    return this.request(`/wallet/fund/complete/${reference}`, "POST", undefined, undefined, authToken)
  }

  getWalletTransactions(authToken: string): Promise<ApiResponse<Transaction[]>> {
  ...
  getMyCreatedGroups(authToken: string): Promise<ApiResponse<Group[]>> {
    return this.request("/groups/my-created", "GET", undefined, undefined, authToken)
  }

  // Get groups the user has joined (not created)
  getMyJoinedGroups(authToken: string): Promise<ApiResponse<Group[]>> {
    return this.request("/groups/my-joined", "GET", undefined, undefined, authToken)
  }

  getGroupDetails(groupId: string, authToken: string): Promise<ApiResponse<Group>> {
    return this.request(`/groups/${groupId}`, "GET", undefined, undefined, authToken)
  }

  deleteGroup(groupId: string, authToken: string): Promise<ApiResponse> {
  return this.request(`/groups/${groupId}`, "DELETE", undefined, undefined, authToken)
}
}
