...
export default function DashboardPage() {
    ...
    useQuery<number>({
      ...
      queryFn: () =>
        api
          ...
          .then(
            (data: ApiResponse<{ balance: number }>) => data?.data?.balance || 0
          ),
  ...
  const { data: activeGroupsData = [], isLoading: isLoadingActiveGroups } =
    useQuery<Group[]>({
      ...
      queryFn: () =>
        api.getActiveGroups(token ?? "").then((res) => res.data || []),
      enabled: !!token,
    });
  ...
  >({
    ...
    queryFn: () =>
      api.getMyJoinedGroups(token ?? "").then((res) => res.data || []),
    enabled: !!token,
  });
  ...
  const handleJoinGroup = async (
    ...
    try {
      console.log("Joining group:", { groupId, slots, paymentMethod });

      // TODO: Implement actual group joining logic
      // await api.joinGroup(groupId, { slots, paymentMethod }, token ?? "");
  ...
  return (
    <div className="px-4 py-6 sm:px-0">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Here's your livestock overview</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card className="bg-green-50 shadow-md">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Available fund</p>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-md">
          ...
          <CardHeader>
            ...
            <CardDescription>
              Popular groups with high activity - Click to join!
            </CardDescription>
          </CardHeader>
              ...
              >
                ...
                <div className="flex justify-between text-sm text-muted-foreground">
                  <span>
                    Investment: ₦
                    {(
                      (group.slotPrice || 0) * group.userSlots!
                    ).toLocaleString()}
                  </span>
